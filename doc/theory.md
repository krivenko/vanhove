...
Beside the logarithm sits a term in $\Delta^n$ with no logarithm on it, and that
one is *analytic*. It is no part of the singular structure and is not reported as
such. Nor does it have to be chosen: an infinite limit would have left it
undetermined — a free constant no value of which is right for two boxes, two
chains and two smooth constants at once — but the finite limit computes it, and it
goes to the regular part along with everything else the asymptotics does not
claim.
...
So the smooth remainder of $S_p \ast S_q$ is never named. It lands in $R$ along
with the subleading singular terms, since only the leading local form of each pair
is derived and anything with $r \geq 2$ is discarded as smooth enough.
...
| --- | --- | --- | --- |
| $10^{-2}$ | $-0.09827092424538$ | $-0.09827092424538$ | $-0.001096$ |
| $10^{-3}$ | $-0.09827142222139$ | $-0.09827142222139$ | $-0.000998$ |
| $10^{-4}$ | $-0.09827142901943$ | $-0.09827142901943$ | $-0.001064$ |

Three things to read off it. $R$ tends to a finite limit rather than to zero, part
of which is the scale convention: a derived singularity measures its distance in
units of one, while the square lattice writes its own logarithm in units of $16t$.
The approach to that limit goes as $d^2\ln d$ — a constant ratio over three
decades — which is exactly the subleading term the exponent ceiling discards,
$C^1$ with a second derivative that diverges logarithmically, and mild enough that
the fit still reaches $10^{-14}$.

And $R$ is not a number at the singular point at all: there the convolution and
the term subtracted from it are both infinite. That is why the Chebyshev nodes lie
strictly inside each panel. A panel boundary sits on every singular point, and the
fit never asks what $R$ is at one.
